 ...
 * @argc: the number of arguments suplied to the program
...
int main(int argc, char *argv[])
{
	...
	if (argc != 3)
	{
		dprint(STDERR_FILENO, "Usage; cp file_from file _to\n"):
			exit(97);
	}
	...
	do {
		if (from == -1 || r == -1)
		{
			...
			dprint(STDERR_FILENO, "Eroor: Can't read from file %s\n", argv[1]);
			free(buffer);
			exit(98);
		}
		...
		r = read(from, buffer, 1024);
		to = open(argc[2], O_WRONLY | O_APPEND);
